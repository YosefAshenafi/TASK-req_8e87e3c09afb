...
export function makeFullContext(): FullContext {
  ...
  const presence = new PresenceService(broadcast, tab, auth);
  const comments = new CommentService(db, broadcast, tab, auth, telemetry, toast, presence);
  const mutualHelp = new MutualHelpService(db, broadcast, telemetry, auth, presence, chat);
